...
async function doScore() {
  ...
  const rl = r.reality;
  $("#reality").innerHTML = rl ? `Kontrola reality (replay ${rl.games} her, ${new Date(rl.createdAt).toLocaleDateString("cs-CZ")}): `
    + rl.byRank.map((b) => `rank ${b.bucket}: predikce ${(b.meanP * 100).toFixed(1)} % → realita <b>${(b.wr * 100).toFixed(1)} %</b> (n ${b.n})`).join(" · ")
    + " — jak dopadly skutečné picky hráčů podle pořadí v našem žebříčku; rozdíl nahoře je selekce maxima, kterou model nezakrývá." : "";
  ...
  $("#recs tbody").innerHTML = r.recommendations.map((x) => {
    ...
    <td>${x.contributions.filter((c) => c.kind !== "strength").map((c) => `<span class="term ${c.logOdds >= 0 ? "pos" : "neg"}" title="${c.games} her${c.attr ? `, z toho atributy soupeře ${c.attr >= 0 ? "+" : ""}${c.attr.toFixed(2)}` : ""}">${KIND[c.kind]} ${c.vsName ?? ""}${c.vsPos ? " " + POS_CS[c.vsPos] : ""} ${c.logOdds >= 0 ? "+" : ""}${c.logOdds.toFixed(2)}</span>`).join("")}</td>
